"""
バックテストエンジン (Version 15.4)
フィルターの動的ON/OFF最適化機能を実装。
"""
import numpy as np
import pandas as pd
import logging
...
def optimize_parameters(df: pd.DataFrame, df_15m: pd.DataFrame, side: str,
    ...
    for _ in range(iterations):
        ...
        p = {
            ...
            'use_rsi_filter': random.choice([True, False]), # RSIフィルターON/OFF
            'use_vwap_filter': random.choice([True, False]) # VWAPフィルターON/OFF
        }
        ...
        if signals.any():
            for i in np.where(signals)[0]:
                ...
                if atr <= 0: continue
                # スクリーニング段階でもフラグを簡易適用
                rsi_v = df['rsi_14'].iloc[i]; vwap_v = df['vwap_dev'].iloc[i]
                if p['use_rsi_filter'] and ((side == 'long' and rsi_v >= 70) or (side == 'short' and rsi_v <= 30)): continue
                if p['use_vwap_filter'] and ((side == 'long' and vwap_v >= 2.5) or (side == 'short' and vwap_v <= -2.5)): continue
